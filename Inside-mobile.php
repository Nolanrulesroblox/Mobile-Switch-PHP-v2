...
if ($tablet_browser > 0) {
   ...
   $path .=  /* a div does NOT work here. if you want to use a div, Look in Mobile.php */ 'file';
   include_once($path);
}
   ...
   $path = $_SERVER['DOCUMENT_ROOT'];
   $path .= header('Location: domain.com/file/index.php');
   include_once($path);
}
else {
   $device = 
   $path = $_SERVER['DOCUMENT_ROOT'];
   $path .= header('Location: http://google.com'); 
   include_once($path);
}   
