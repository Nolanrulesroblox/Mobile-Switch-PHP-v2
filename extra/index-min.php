...
$tablet_browser = 0;
$mobile_browser = 0;
 if (preg_match('/(tablet|ipad|playbook)|(android(?!.*(mobi|opera mini)))/i', strtolower($_SERVER['HTTP_USER_AGENT']))){ $tablet_browser++;} if (preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|phone|android|iemobile)/i', strtolower($_SERVER['HTTP_USER_AGENT']))){ $mobile_browser++;} if ((strpos(strtolower($_SERVER['HTTP_ACCEPT']),'application/vnd.wap.xhtml+xml') >0) or ((isset($_SERVER['HTTP_X_WAP_PROFILE']) or isset($_SERVER['HTTP_PROFILE'])))){ $mobile_browser++;}
...
if ($tablet_browser >0){ $device=$path=$_SERVER['DOCUMENT_ROOT']; $path .="/demo/rscs/desktop.php"; include_once($path);}
else if ($mobile_browser >0){ $device=$path=$_SERVER['DOCUMENT_ROOT']; $path .="/demo/rscs/mobile.php"; include_once($path);}
else{ $device=$path=$_SERVER['DOCUMENT_ROOT']; $path .="/demo/rscs/desktop.php"; include_once($path);} ?>
<?php 
// Replace $path .= "link-file-.freecandy" // (C) 2020 NRRINC Media nrrinc.net Apachev2 http://www.apache.org/licenses/
?>